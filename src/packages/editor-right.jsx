...
export default defineComponent({
  ...
  setup(props){
    ...
    return ()=>{
      if(props.lastSelectBlock && props.lastSelectBlock.animate.name == undefined) isShowAddButton.value = true
      //属性
      if(props.rightActiveIndex.value==1){
        return (
            <EditorOperator commands={props.commands} block={props.lastSelectBlock} data={props.data.value}></EditorOperator>
        )
      }
      //动画
          ...
          <div>
            { //显示时=当前组件的animate为空,此时显示可用操作
              isShowAddButton.value && <>
                <div>
                  <ElButton icon="Plus" type="primary" plain onClick={()=>{isShowAllAnimate.value = true,isShowAddButton.value = false}} >添加动画</ElButton>
                </div>
              </>
            }
            {
              ...
              <div class="animate">
                {
                  animateObj.map((animate)=>{
                    return(
                      <div onMouseenter={()=>animateEnter(animate.value)} onMouseleave={()=>{animateLeave(animate.value)}} class="animateItem" onClick={()=>animateClick(animate.value)} >
                        <p>{animate.label}</p>
                      </div>
                    )
                  })
            ...
            {
              props.lastSelectBlock && props.lastSelectBlock.animate.name !=undefined && <>
              <div>
