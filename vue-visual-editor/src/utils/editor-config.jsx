// 列表区可以显示所有的元素
import { ElButton, ElInput, ElOption, ElSelect, ElDatePicker, ElRadio, ElSlider, ElProgress,ElCheckbox,ElLink,ElCheckboxGroup } from 'element-plus'
import 'element-plus'
...
registerConfig.register({
  ...
  render: ({ props}) => 
      <ElDatePicker size={props.size} type={props.type} placeholder="选择日期">
	</ElDatePicker>,
  key: 'datePicker',
  props: {
...
registerConfig.register({
  label: '单选框',
  preview: () => <ElRadioGroup><ElRadio label="预览1"></ElRadio><ElRadio label="预览2"></ElRadio></ElRadioGroup>,
  render: ({ props, model }) => {
    return (
...
registerConfig.register({
  label: '滑块',
  preview: () => <ElSlider modelValue="" placeholder="预览"></ElSlider>,
  render: ({ props, model }) => {
    return (
      <ElSlider {...model.default} size={props.size} show-input={props.showInput} placement={props.placement} step={props.step} style={{width: (props.width || '200')+"px"}}></ElSlider>
    )
  },
  key: 'slider',
  animate:{},
...
registerConfig.register({
  ...
  render: ({ props}) => 
      <ElProgress percentage={props.percentage||50} text-inside={props.textInside} indeterminate={props.indeterminate} style={{width: (props.width || '200')+"px"}} duration={props.duration} status={props.status}>
	</ElProgress>,
  key: 'progress',
  props: {
